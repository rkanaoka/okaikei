...
async function main() {
  ...
  // ── Tables ─────────────────────────────────────────────────────────────────
  const tableLabels = [
    ...Array.from({ length: 8 }, (_, i) => `Mesa ${i + 1}`),   // 1-8
    'Balcão 1',                                                 // 9  (era "Balcão")
    'Delivery',                                                 // 10
    ...Array.from({ length: 41 }, (_, i) => `Mesa ${i + 10}`),  // 11-51 -> Mesa 10..Mesa 50
    'Balcão 2',                                                 // 52
    'Mesa Externa 1',                                            // 53
    'Mesa Externa 2',                                            // 54
  ];
  ...
  for (let i = 0; i < tableLabels.length; i++) {
    await prisma.table.upsert({
      ...
      create: { id: uuidv7(), number: i + 1, label: tableLabels[i] },
    });
  }
  ...
  // MenuItem não tem @@unique em name, então usa createMany com skipDuplicates
  // (não é possível via upsert sem campo único — inserimos apenas o que ainda não existe)
  ...
  if (toInsert.length > 0) {
    await prisma.menuItem.createMany({
      data: toInsert.map(m => ({ id: uuidv7(), ...m, available: true })),
      skipDuplicates: true,
    });
  }
  ...
  // ── Printers ───────────────────────────────────────────────────────────────
  // Schema: category (unique), ip, port, label, enabled
  const printers = [
  ...
  for (const p of printers) {
    await prisma.printer.upsert({
      ...
      update: { ip: p.ip },
      create: { id: uuidv7(), ...p, enabled: true },
    });
  }
  ...
  // ── System config ──────────────────────────────────────────────────────────
  // Schema: key (PK String), value (Json), updatedAt (auto)
