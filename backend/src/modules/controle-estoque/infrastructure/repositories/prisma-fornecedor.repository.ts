...
import { FornecedorRepositoryPort, FornecedorDadosOpcionais } from '@/modules/controle-estoque/domain/repositories/fornecedor-repository.port';
...
export class PrismaFornecedorRepository implements FornecedorRepositoryPort {
  ...
  create(data: { id: string; nome: string; cnpj?: string | null } & FornecedorDadosOpcionais) {
    return (this.prisma as any).fornecedor.create({ data });
  }

  update(id: string, data: Partial<{ nome: string; cnpj: string | null; active: boolean } & FornecedorDadosOpcionais>) {
